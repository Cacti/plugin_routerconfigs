...
function edit_devices() {
	...
	draw_edit_form(
		[
			'config' => ['no_form_tag' => true, 'form_name' => 'chk'],
...
function show_devices() {
	...
	if (cacti_sizeof($devicetypes)) {
		foreach ($devicetypes as $devicetype) {
			print "<option value='" . $devicetype['id'] . "'";

			if (get_request_var('devicetype') == $devicetype['id']) {
				print ' selected';
			} print '>' . htmlspecialchars($devicetype['name']) . "</option>\n";
		}
	}
	?>
						</select>
	...
	if (cacti_sizeof($accounts)) {
		foreach ($accounts as $account) {
			print "<option value='" . $account['id'] . "'";

			if (get_request_var('account') == $account['id']) {
				print ' selected';
			} print '>' . htmlspecialchars($account['name']) . "</option>\n";
		}
	}
	?>
						</select>
